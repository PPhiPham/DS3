...
def extract_num_unit_patterns(text):
    return re.findall(r'\b\d+(?:/\d+)?\s?(?:ft|in|inch|inches|cm|mm|m|x|lbs|lb|oz|g|kg)?\b', str(text).lower())

def count_common_num_unit(list1, list2):
...
print("Extracting number-unit features...")
df_all['search_num_units'] = df_all['search_term'].map(extract_num_unit_patterns)
df_all['desc_num_units'] = df_all['product_description'].map(extract_num_unit_patterns)
df_all['title_num_units'] = df_all['product_title'].map(extract_num_unit_patterns)
df_all['num_unit_match_desc'] = df_all.apply(lambda row: count_common_num_unit(row['search_num_units'], row['desc_num_units']), axis=1)
df_all['num_unit_match_title'] = df_all.apply(lambda row: count_common_num_unit(row['search_num_units'], row['title_num_units']), axis=1)
print("Number-unit features extracted.")
...
rf = RandomForestRegressor(n_estimators=17, max_depth=9, max_features=None, min_samples_leaf=1, min_samples_split=8, random_state=0)
